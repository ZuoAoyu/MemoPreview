...
// 静态正则表达式，避免重复编译
static QRegularExpression s_whitespaceRegex("\\s+");
...
bool isBlockElement(const QString &tag) {
  static const QSet<QString> blocks = {"p",  "div", "ul", "ol", "li", "h1",
                                       "h2", "h3",  "h4", "h5", "h6"};
...
void appendNodeText(IHTMLDOMNode *pNode, QString &result) {
  ...
  if (nodeType == 3) { // NODE_TEXT
    ...
    if (SUCCEEDED(pNode->get_nodeValue(&v)) && v.vt == VT_BSTR && v.bstrVal) {
      ...
      // 合并多余空白
      text = text.replace(s_whitespaceRegex, " ");

      QString trimmed = text.trimmed();
      if (trimmed.isEmpty())
        return;

      // 优化：简化父节点检查逻辑，只在结果为空时检查
      // 大多数情况下result已有内容，无需查询父节点
      if (result.isEmpty()) {
        // 只在首个文本节点时才检查父节点是否为body
        IHTMLDOMNode *pParent = nullptr;
        if (SUCCEEDED(pNode->get_parentNode(&pParent)) && pParent) {
          IHTMLElement *pParentElem = nullptr;
          if (SUCCEEDED(pParent->QueryInterface(IID_IHTMLElement,
                                                (void **)&pParentElem)) &&
              pParentElem) {
            BSTR bTag = nullptr;
            if (SUCCEEDED(pParentElem->get_tagName(&bTag)) && bTag) {
              QString parentTag = bstrToQString(bTag).toLower();
              if (parentTag == "body") {
  ...
  if (nodeType == 1) { // NODE_ELEMENT
    ...
    if (SUCCEEDED(pNode->QueryInterface(IID_IHTMLElement, (void **)&pElem)) &&
        pElem) {
      BSTR bTag = nullptr;
...
std::vector<HWND>
SuperMemoIeExtractor::enumerateIeControls(HWND superMemoHwnd) {
...
bool SuperMemoIeExtractor::extractControlContent(HWND hwnd,
                                                 IeControlContent &info) {
  char className[256] = {0};
  char winText[256] = {0};
  GetClassNameA(hwnd, className, sizeof(className) - 1);
  ...
  // 对于一个 SuperMemo Element，如果为其添加了 Reference，Element
  // 末尾会有相关的字符串，需要在编译 PDF 文档时将其去除
...
QString SuperMemoIeExtractor::getDocumentContent(HWND hwnd, QString &title,
                                                 QString &url) {
  QString result;
  ...
  DWORD_PTR dwRes = 0;
  LRESULT lRes =
      SendMessageTimeout(hwnd, nMsg, 0, 0, SMTO_ABORTIFHUNG, 1000, &dwRes);
  ...
  // 最后兜底：尝试documentElement
  if (result.trimmed().isEmpty()) {
    IHTMLDocument3 *pDoc3 = nullptr;
    if (SUCCEEDED(pDoc->QueryInterface(IID_IHTMLDocument3, (void **)&pDoc3)) &&
        pDoc3) {
